process mmseqs2 {
  ...
  mmseqs search ${query.simpleName} ${target.simpleName} \${RESULTDB} ${tmp.simpleName} --threads ${params.threads} -e ${params.evalue} --min-aln-len ${params.minalnlen} --min-seq-id ${params.pident}
  mmseqs convertalis --threads ${params.threads} --format-mode 0 --format-output 'query,target,pident,alnlen,mismatch,gapopen,qlen,qstart,qend,tstart,tend,evalue,bits' ${query.simpleName} ${target.simpleName} \${RESULTDB} \${OUT}
  ...
  rm -rf ${tmp}/
  rm ${query.simpleName}* ${target.simpleName}*
  """
